...
public class Turma extends Observable {
    ...
    public Set<Aluno> listaPassaram(){
        Set<Aluno> lista = new TreeSet<Aluno>();
        Aluno estudante = null;
        for(Map.Entry<String, Aluno> e : turma.entrySet()){
            estudante = (Aluno) e.getValue().clone();
            if(estudante.passa()==true) {lista.add(estudante); }
              }
        return lista;
    }
