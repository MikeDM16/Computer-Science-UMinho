...
GLuint vertexCount, buffers[2], vertices ;
...
void prepareCilinder(float height, float radius, int sides) {
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertexCount, normalB, GL_STATIC_DRAW);
...
void initLighting() {
	...
	float redish[4] = { 0.6f, 0.1f, 0.1f, 0.8f };
	//glMaterialfv(GL_FRONT, GL_SPECULAR, redish);
