...
void cylinder0(float radius, float height, int sides) {
	...
	float* vertexB;
	int size = sides * (12) * 3 * (sizeof(float));
	int i;
	float step;
	...
	int iter = 0;
	vertexB[iter++] = 0;
	vertexB[iter++] = height*0.5;
	vertexB[iter++] = 0;
	for (i=0; i <= sides; i++) {
			vertexB[iter++] = cos(i * step * M_PI / 180.0)*radius;
			vertexB[iter++] = height*0.5;
			vertexB[iter++] = -sin(i * step *M_PI / 180.0)*radius;
	}
	...
	glDrawArrays(GL_TRIANGLES, 0, sides * 12);
}
